...
.picker-wrapper .top-content .confirm-btn{
  float: right;
}
.picker-wrapper .picker-content{
  background: #eee;
...
<template>
  <div>
    <div class="flex align-center">
      <span>{{msg}}</span>
      <div class="collapse">
        <div class="collapseArrow right"></div>
      </div>
    </div>
      ...
      <div class="picker-backdrop" v-show="value" @click="close">
        <div class="picker-wrapper">
          <div class="top-content">
            <span>{{title}}</span>
            <span class="confirm-btn" @click="close">确定</span>
          </div>
	...
	export default {
      ...
      msg:{
		...
		methods : {
			...
			close (...initialArgs) {
				let e = initialArgs[0];
				if (e.target && e.target.classList.contains('picker-backdrop')||e.target.classList.contains('confirm-btn')) {
          e.stopPropagation();
          ...
          this.$emit('closePicker');
			...
			sphereScroller (el) {
                ...
                el.style['-webkit-transform'] = `translate3d(0, 0, -110px) rotateX(${(value - 18 * i)}deg)`;
						...
						get () {
							...
							return Number(((firstEl.style['-webkit-transform'] || '').match(/rotateX\((\-?\d+(?:\.\d+)?)deg\)/) || [0, 0])[1]);
						}
					}
				});
